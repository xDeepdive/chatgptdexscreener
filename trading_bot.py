...
def fetch_tokens():
    """
    Fetch the latest token profiles from the Dexscreener API.
    """
    try:
...
def filter_tokens(tokens):
    ...
    for token in tokens:
        try:
            ...
            address = token.get("tokenAddress", None)
            market_cap = token.get("marketCapUSD", 0)  # Assuming market cap is provided in USD
            liquidity = token.get("liquidityUSD", 0)  # Assuming liquidity is provided in USD
            socials = token.get("links", [])

            # Check for required social links
